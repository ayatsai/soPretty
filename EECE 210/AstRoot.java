...
public class AstRoot extends ScriptNode implements Cloneable {
    ...
    private class AstNodeGenerator_Depth implements Iterator {
    	 ...
    	 */
    	private DepthCollectionVisitor dcv;
    	...
    	public AstNodeGenerator_Depth(AstRoot rt) {
    		this.astRt = rt;
    		next = (AstNode)astRt;
    		dcv = new DepthCollectionVisitor();
    		visit(dcv);
    		
    		nodeList = dcv.getNodeList();
    		curIndex = 0;
		...
		public Object next() throws NoSuchElementException {
			...
			if (curIndex >= nodeList.size()) throw new NoSuchElementException("AstRoot.next()");
			AstNode currentNode = nodeList.get(curIndex);
			
			curIndex++;
			return currentNode; //EECE310_TODO: Delete this line once you've fully implemented this function
		}
		 ...
		 */
		protected class DepthCollectionVisitor implements NodeVisitor {
			private ArrayList<ArrayList<AstNode>> nodeList;
			
	        public DepthCollectionVisitor() {
	        	nodeList = new ArrayList<ArrayList<AstNode>>();   	
	        }
	     
	        public boolean visit(AstNode node) {
	        	try {
	        		nodeList.get(node.depth()).add(node);
	        	} catch (Exception e) {
	        		nodeList.add(new ArrayList<AstNode>());
	        		nodeList.get(node.depth()).add(node);
	        	}
	        ...
	        public ArrayList<AstNode> getNodeList() {
	        	ArrayList<AstNode> compiledList = new ArrayList<AstNode>();
	        	for (int i = 0; i < nodeList.size(); i++) {
	        		compiledList.addAll(nodeList.get(i)); 
	        	}
	        	return compiledList;
	        }
	    }
